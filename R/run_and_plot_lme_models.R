...
## By Subject ##
individual_and_overall_lme_onset_combval_turnaround_model_and_plot <- function(region, freq, sub, color_sub, distance_df, brain_df, predictor, plot_title, y_low, y_high, rerun_model = TRUE){
  ...
  if(rerun_model == TRUE){
    ...
    results <- foreach(step=sort(unique(brain_behave_lme_df$turn_time)), .combine=rbind, .packages=c("dplyr", "robustlmm", "lmerTest", "tibble")) %dopar% {
      ...
      # fit the model with random effects of subject and electrode
      formula_pred <- as.formula(paste0("theta ~ ", predictor, " + (1|elec_id)"))
      # model_all <- summary(robustlmm::rlmer(formula_pred, data = time_step_df))
      model_all <- summary(lmerTest::lmer(formula_pred, data = time_step_df))
    ...
    # save for later #
    write_csv(value_results_only, path(here(), "munge", "lme_model_results", paste(region, freq, "model_results", predictor, sub, "turnaround_all_subs_robust.csv", sep = "_")))
    
  } else{
    
    # load previous model
    value_results_only <- read_csv(path(here(), "munge", "lme_model_results", paste(region, freq, "model_results", predictor, sub, "turnaround_all_subs_robust.csv", sep = "_")))
  ...
  # save plot #
  ggsave(path(here(), "figures", "linear_mixed_effect_models", paste(region, freq, "trial", predictor, sub, "turnaround_indv_subs.png", sep = "_")),
         plot = plot,
         width = 12,
         height = 10,
         units = "in",
...
individual_and_overall_robust_interact_lme_turnaround_model_and_plot <- function(region, freq, distance_df, brain_df, threat_pred, reward_pred, plot_title, y_low, y_high, rerun_model = TRUE){
  ...
  if(rerun_model == TRUE){
    ...
    brain_behave_lme_df <- brain_behave_df %>%
      ...
      filter(turn_time > -2 & turn_time < 2) %>%
      # select and scale the necessary variables for the model
      select(subject, elec_id, theta, trial_numeric, turn_time, UserLocation, GhostLocation, ghost_direction, points_remaining, distance_to_ghost)  %>%
      mutate(UserLocation = scale(UserLocation)) %>%
      mutate(GhostLocation = scale(GhostLocation)) %>%
      mutate(points_remaining = scale(points_remaining)) %>%
      mutate(distance_to_ghost = scale(distance_to_ghost))
    ...
    results <- foreach(step=sort(unique(brain_behave_lme_df$turn_time)), .combine=rbind, .packages=c("dplyr", "robustlmm", "lmerTest", "tibble")) %dopar% {
      ...
      interaction_results <- rbind(interaction_results, 
                                   tibble(
                                     ...
                                     `Std. Error` = model_all$coefficients[4, "Std. Error"],
                                     `t value` = model_all$coefficients[4, "t value"],
                                     `Pr(>|t|)` = model_all$coefficients[4, "Pr(>|t|)"]
                                   ))
