individual_and_overall_robust_lme_onset_before_turn_model_and_plot <- function(region, freq, distance_df, brain_df, plot_title, y_low, y_high, rerun_model = TRUE){
  ...
  if(rerun_model == TRUE){
    ...
    results <- foreach(step=sort(unique(brain_behave_lme_df$move_time)), .combine=rbind, .packages=c("dplyr", "robustlmm", "lmerTest", "MASS", "tibble")) %dopar% {
...
individual_and_overall_lme_onset_before_turn_model_and_plot <- function(region, freq, distance_df, brain_df, plot_title, y_low, y_high, rerun_model = TRUE){
  ...
  if(rerun_model == TRUE){
    ...
    for(step in sort(unique(brain_behave_lme_df$move_time))) {
      ...
      # fit the model with random effects of subject and electrode
      model_all <- summary(lmerTest::lmer(theta ~ distance_to_ghost + points_remaining + (1|subject/elec_id), data = time_step_df))
      ...
      for(elec in unique(time_step_df$elec_id)){
        ...
        try({
          ...
          model_sub <- summary(lm(theta ~ distance_to_ghost + points_remaining, data = time_step_df %>% filter(elec_id == elec)))
          ...
          points_r_results <- rbind(points_r_results, 
                                    tibble(
                                      case = elec,
                                      time = step, 
                                      Estimate = model_sub$coefficients[3, "Estimate"],
                                      `Std. Error` = model_sub$coefficients[3, "Std. Error"],
                                      `t value` = model_sub$coefficients[3, "t value"],
                                      `Pr(>|t|)` = model_sub$coefficients[3, "Pr(>|t|)"]
                                    )) 
        }, TRUE)
      }
    ...
    # save for later #
    write_csv(model_results_onset_before_turn_all_subs, path(here(), "munge", "lme_model_results", paste(region, freq, "model_results_onset_before_turn_all_subs.csv", sep = "_")))
    
  } else{
    
    # load previous model
    model_results_onset_before_turn_all_subs <- read_csv(path(here(), "munge", "lme_model_results", paste(region, freq, "model_results_onset_before_turn_all_subs.csv", sep = "_")))
  ...
  # save plot #
  ggsave(path(here(), "figures", "linear_mixed_effect_models", paste(region, freq, "trial_onset_before_turn_indv_subs.png", sep = "_")),
         plot = plot,
         width = 12,
         height = 10,
         units = "in",
...
individual_and_overall_lme_onset_turnaround_model_and_plot <- function(region, freq, distance_df, brain_df, plot_title, y_low, y_high, rerun_model = TRUE){
  ...
  if(rerun_model == TRUE){
    ...
    # loop over time
    for(step in sort(unique(brain_behave_lme_df$turn_time))){
      ...
      # fit the model with random effects of subject and electrode
      model_all <- summary(lmer(theta ~ distance_to_ghost + points_remaining + (1|subject/elec_id), data = time_step_df))
    ...
    # save for later #
    write_csv(model_results_onset_turnaround_all_subs, path(here(), "munge", "lme_model_results", paste(region, freq, "model_results_onset_turnaround_all_subs.csv", sep = "_")))
    
  } else{
    
    # load previous model
    model_results_onset_turnaround_all_subs <- read_csv(path(here(), "munge", "lme_model_results", paste(region, freq, "model_results_onset_turnaround_all_subs.csv", sep = "_")))
  ...
  # save plot #
  ggsave(path(here(), "figures", "linear_mixed_effect_models", paste(region, freq, "trial_onset_turnaround_indv_subs.png", sep = "_")),
         plot = plot,
         width = 12,
         height = 10,
         units = "in",
