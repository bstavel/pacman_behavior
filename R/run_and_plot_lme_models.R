...
individual_and_overall_lme_onset_combval_before_turn_model_and_plot <- function(region, freq, distance_df, brain_df, predictor, plot_title, y_low, y_high, rerun_model = TRUE){
  ...
  if(rerun_model == TRUE){
    ...
    brain_behave_lme_df <- brain_behave_df %>%
      ...
      mutate(eaten_threat_diff = rescale(Eaten - distance_to_ghost)) %>%
      mutate(nreward_threat_add = rescale(distance_to_ghost + (-1*points_remaining))) %>%
      mutate(reward_threat_sum = rescale(points_remaining + distance_to_ghost))
