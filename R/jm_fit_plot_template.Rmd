...
for(current_seed in sample(1:100, 1)){
    ...
    jm_fit <- fit_joint_models(train_long_data, train_cox_df, file_name)

    ## Print summary ##
    kable(summary(jm_fit)$Survival, caption = "Joint Model") %>% kable_styling()
    cat(paste0("\n\n"))
    ...
    test_times <- quantile(timepoints[timepoints > .5], probs = c(.2, .4, .5, .6, .8))
...
write_csv(metrics_df, path(here(), "data", "joint_models", "model_summaries", paste0(current_subject, "fit_metrics_base_dir_model_df.csv")))
write_csv(survival_df, path(here(), "data", "joint_models", "model_summaries", paste0(current_subject, "model_params_base_dir_model_df.csv")))
