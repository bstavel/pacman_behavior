...
train_long_data$points_remaining <- as.numeric(train_long_data$points_remaining)

train_long_data$distance_to_ghost <- as.numeric(train_long_data$distance_to_ghost)

write_csv(train_long_data, path(here(), "data/jm_example_train_long_data.csv"))
write_csv(train_cox_df, path(here(), "data/jm_example_train_cox_data.csv"))

train_long_data <- train_long_data %>%
  mutate(trial_numeric = factor(trial_numeric)) %>%
  mutate(trial_ids = paste0("BJH016_", trial_numeric))
...
lm_threat_fc <- lme(distance_to_ghost ~ jm_time, data = train_long_data, 
                 random = ~jm_time | trial_ids, control = lmeControl(opt = 'optim'))
...
write_csv(metrics_df, path(here(), "data", "joint_models", "model_summaries", 
                           paste0(current_subject, "_permuted_fit_metrics_theta_time_model_df.csv")))
write_csv(survival_df, path(here(), "data", "joint_models", "model_summaries", 
                            paste0(current_subject, "_permuted_model_params_theta_time_model_df.csv")))
