...
# get subjects #
sg_sub_df <- sg_df %>% 
  filter(roi_pair %in% c("insula_amyg")) %>%
...
failed_subjects <- c()
for(current_subject in unique(sg_sub_df$subject)){


  tryCatch({
  
    output_file_name <- paste("jm_correl_theta_time_dir_modeling_for_", current_subject, ".html", sep = "")
    
    # Render the R Markdown document, passing the current subject as a parameter
    render(input = path(here(), "R", "jm_fit_ieeg_correlation_theta_plot_template.Rmd"),
