...
example_elec_df <- hc_theta_data %>%
  ...
  distinct() %>%
  ungroup() %>%
  mutate(trial_time = round(trial_time - .5, 2))
...
ex_df <- inner_join(example_elec_df, example_behave_df, by = c("subject", "trial_numeric", "trial_time"))
...
# joint model #
jm_basic_fit <- jm(cox_fit, list(time_model), 
             time_var = "jm_time", data_Surv = cox_df, 
             functional_forms = ~value(mean_theta), #+ slope(mean_theta),
