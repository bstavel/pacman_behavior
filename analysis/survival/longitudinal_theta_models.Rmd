...
## Average Theta - SLCH002

```{r prep-for-eda}

example_elec_df <- ofc_theta_data %>%
  ...
  group_by(subject, trial_time, trial_numeric) %>%
...
example_behave_df <- all_subs_g_dist %>%
  # filter(subject == "SLCH002") %>%
...
ex_df <- full_join(example_elec_df, example_behave_df, by = join_by(subject, trial_numeric, game_time)) %>% 
  filter(!is.na(jm_time)) %>%
  group_by(trial_numeric) %>%
  mutate(jm_time = trial_time - first(trial_time)) %>%
  ungroup()
...
example_behave_df %>%
  group_by(trial_numeric) %>%
  mutate(true_turn_time = turnaround_time + first(game_time)) %>%
...
ex_df <- ex_df %>%
  # mutate(long_short_trial = if_else(is.na(turnaround_time), "discard", if_else(turnaround_time <= 2, "short", "long"))) %>%
  filter(trial_time >= 0) %>%
  ungroup()
...
long_data_df <- ex_df %>%
  ...
  mutate(sin_term = sin(5 * pi * jm_time )) %>%
  mutate(cos_term = cos(5 * pi * jm_time )) %>%
  ungroup() 
...
test_trials <- sample(long_data_df$trial_numeric, 6)
...
long_data_df %>%
  filter(trial_numeric %in% test_trials) %>%
  ggplot(., aes(x = jm_time, y = sin_term, group = trial_numeric)) +
  geom_line() + 
  facet_wrap(~trial_numeric, scale = "free") 
...
long_data_df <- ex_df %>%
  mutate(trial_id = paste0(subject, "_", trial_numeric)) %>%
  mutate(jm_time = round(jm_time, 2)) %>%
  filter(jm_time <= turnaround_time) %>%
  filter(!is.na(mean_theta)) %>%
  # mutate(jm_time = scale(jm_time)[,]) %>%
  mutate(mean_theta = scale(mean_theta)[,]) %>%
  mutate(sin_term = sin(7 * pi * jm_time )) %>%
  mutate(cos_term = cos(7 * pi * jm_time )) 
  

# fit model #
sin_cos_time_model <- lme(mean_theta ~ jm_time + sin_term + cos_term,
                     data = long_data_df,
                     random = ~ sin_term + cos_term + jm_time | trial_id,
                     control = lmeControl(opt = "optim", msMaxIter = 200))


sin_cos_model <- lme(mean_theta ~ jm_time + sin_term + cos_term,
                     data = long_data_df,
                     random = ~ sin_term + cos_term | trial_id,
                     control = lmeControl(opt = "optim", msMaxIter = 200))

sin_model <- lme(mean_theta ~ jm_time + sin_term ,
                     data = long_data_df,
                     random = ~ jm_time | trial_id,
                     control = lmeControl(opt = "optim", msMaxIter = 200))

lin_model <- lme(mean_theta ~ jm_time + cos_term + sin_term,
                 data = long_data_df,
                 random = ~ jm_time | trial_id,
                 control = lmeControl(opt = "optim", msMaxIter = 200))
...
summary(sin_cos_time_model)

# save predictions #
long_data_df$prediction <- predict(sin_cos_time_model, long_data_df, type = "response")

# plot predictions #
long_data_df %>%
  filter(trial_id %in% sample(trial_id, 6)) %>%
  arrange(turnaround_time) %>%
  mutate(trial_numeric = factor(trial_numeric, levels = unique(trial_numeric))) %>%
...
cox_df <- long_data_df %>%
  ...
  ungroup() %>%
  mutate(event = 1) %>%
  # mutate(trial = factor(trial_numeric)) %>%
  select(trial_id, turnaround_time, event, EVENT) 
...
# joint model #
jm_basic_fit <- jm(cox_fit, list(sin_cos_time_model), 
             time_var = "jm_time",  data_Surv = cox_df, 
             functional_forms = ~value(mean_theta),
             n_burnin = 100000, n_iter = 200000, n_chains =8, cores = 8)
