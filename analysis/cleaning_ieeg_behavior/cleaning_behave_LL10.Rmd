...
# task vars df #
ll10_all_vars_df <- get_across_task_variables_bci(ll10_data_clean)
...
no_biscuits_trials <- ll10_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
write_csv(bad_df, path(here(), './data/ieeg_behave/LL10_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_trials <- read_csv(path(here(), './data/ieeg_behave/LL10_bad_trials.csv'))

create_last_away_events(ll10_data_clean, 'LL10_last_away_events.csv')
create_first_dot_events(ll10_data_clean, 'LL10_first_dot_events.csv')
create_first_move_events(ll10_data_clean, 'LL10_first_move_events.csv')
create_died_events(ll10_data_clean, 'LL10_died_events.csv')
...
# read in bad df
bad_df <- read_csv(path(here(), './data/ieeg_behave/LL10_bad_trials.csv'))


trial_times <- ll10_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
