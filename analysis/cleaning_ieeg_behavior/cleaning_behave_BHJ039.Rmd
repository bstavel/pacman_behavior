...
# on trial 19 they didn't really die
died_events <- read_csv(path(here(), './data/ieeg_behave/BJH039_died_events.csv'))
died_events <- died_events %>%
  mutate(died = if_else(neural_trial_numeric == 19, 0, died))
write_csv(died_events, path(here(), './data/ieeg_behave/BJH039_died_events.csv'))
```
