...
tmp <- last_dot_df <- sl02_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
  filter(!(neural_trial_numeric %in% bad_trials$neural_trial_numeric)) 

last_dot_df <- sl02_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
  filter(!(neural_trial_numeric %in% bad_trials$neural_trial_numeric)) %>%
...
last_dot_trial_end <- sl02_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
  filter(!(neural_trial_numeric %in% bad_trials$neural_trial_numeric)) %>%
...
write_csv(last_dot_clean_df, path(here(), './data/ieeg_behave/SLCH002_last_dot_events.csv'))
...
write_csv(last_away_clean_df, path(here(), './data/ieeg_behave/SLCH002_last_away_events.csv'))
