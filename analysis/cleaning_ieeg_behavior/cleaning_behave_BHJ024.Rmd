...
bjh24_data <- read_csv(path(here(), './data/ieeg_behave/BJH024_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# clean Biscuits #
bjh24_data_clean <- clean_bci_data(bjh24_data, 2000)

# create distance df #
bjh24_data_distance <- create_distance_df_bci(bjh24_data_clean)

# task vars df #
bjh24_all_vars_df <- get_across_task_variables_bci(bjh24_data_clean)
...
# save info
no_biscuits_trials <- bjh24_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
## looks like there was another long trial from hanging out after a ghost trial
...
bad_trials <- unique(c(no_biscuits_trials, long_trials, paused_trials))
...
length(unique(bjh24_data_clean$trial_numeric))
...
write_csv(last_away_clean_df, path(here(), './data/ieeg_behave/BJH024_last_away_events.csv'))
