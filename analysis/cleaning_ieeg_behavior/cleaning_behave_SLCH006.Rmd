...
slch6_data <- read_csv(path(here(), './data/ieeg_behave/SLCH006_raw_behave.csv'), col_types = cols(X1 = col_double(),
...
# clean Biscuits #
slch6_data_clean <- clean_bci_data(slch6_data, 2000)

# create distance df #
slch6_data_distance <- create_distance_df_bci(slch6_data_clean)

# task vars df #
slch6_all_vars_df <- get_across_task_variables_bci(slch6_data_clean)
...
no_biscuits_trials <- slch6_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
write_csv(bad_df, path(here(), './data/ieeg_behave/SLCH006_bad_trials.csv'))
...
create_last_away_events(slch6_data_clean, 'SLCH006_last_away_events.csv')
create_first_dot_events(slch6_data_clean, 'SLCH006_first_dot_events.csv')
create_first_move_events(slch6_data_clean, 'SLCH006_first_move_events.csv')
create_died_events(slch6_data_clean, 'SLCH006_died_events.csv')
...
# read in bad df
bad_df <- read_csv(path(here(), './data/ieeg_behave/SLCH006_bad_trials.csv'))


trial_times <- slch6_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
