...
LL16_data <- read_csv(path(here(), './data/ieeg_behave/LL16_raw_behave.csv'), col_types = cols(`...1` = col_double(),
...
# clean Biscuits #
LL16_data_clean <- clean_bci_data(LL16_data, 512)

# create distance df #
LL16_data_distance <- create_distance_df_bci(LL16_data_clean)

# task vars df #
LL16_all_vars_df <- get_across_task_variables_bci(LL16_data_clean)
...
no_biscuits_trials <- LL16_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
## looks like 161 (160 in neural_trial_numeric) is also paused
paused_trials <- c(160)
...
length(unique(LL16_data_clean$trial_numeric))
...
bad_trials <- c(no_biscuits_trials, long_trials, paused_trials)

bad_df <- data_frame("neural_trial_numeric" = bad_trials)

write_csv(bad_df, path(here(), './data/ieeg_behave/LL16_bad_trials.csv'))

```

```{r time-lock-events, fig.width = 16, fig.height = 16}

bad_df <- read_csv(path(here(), './data/ieeg_behave/LL16_bad_trials.csv'))

create_last_away_events(LL16_data_clean, 'LL16_last_away_events.csv')
create_first_dot_events(LL16_data_clean, 'LL16_first_dot_events.csv')
create_first_move_events(LL16_data_clean, 'LL16_first_move_events.csv')
create_died_events(LL16_data_clean, 'LL16_died_events.csv')
create_attack_events(LL16_data_clean, 'LL16_attack_events.csv')


# on 80, 60, 99 they didn't really die
died_events <- read_csv(path(here(), './data/ieeg_behave/LL16_died_events.csv'))
died_events <- died_events %>%
  mutate(died = if_else(neural_trial_numeric == c(80, 60, 99), 0, died))
write_csv(died_events, path(here(), './data/ieeg_behave/LL16_died_events.csv'))


# on 82 they were not attacked
attack_events <- read_csv(path(here(), './data/ieeg_behave/LL16_attack_events.csv'))
attack_events <- attack_events %>%
  filter(neural_trial_numeric != 82)
write_csv(attack_events, path(here(), './data/ieeg_behave/LL16_attack_events.csv'))
