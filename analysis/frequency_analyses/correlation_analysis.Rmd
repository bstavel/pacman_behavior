---
title: "Cross Correlation"
output: html_document
date: "2023-06-07"
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo <- FALSE,  # don't print the code chunk
  warning <- FALSE,  # don't print warnings
  message <- FALSE,  # don't print messages
  fig.width <- 5,  # set default width of figures
  fig.height <- 8,  # set default height of figures
  fig.align <- "center",  # always align figure in center
  fig.pos <- "H",  # always plot figure at the exact location of the code chunk
  cache <- FALSE)  # cache results

## libraries ##
library(tidyverse)
library(ggplot2)
library(magrittr)
library(grid)
library(gtable)
library(RColorBrewer)
library(doParallel)
library(parallel)
library(foreach)
library(here)
library(fs)
library(scales)

## hand written functions ##
source(path(here(), "R", 'mutate_cond.R'))
source(path(here(), "R", 'clean_behavioral_data.R'))

## plotting helpers ##
# ggthemr("light")
getPalette = colorRampPalette(brewer.pal(17, "Set1"))
ghost_colors <- c("#E48DB7","#55BBC8", "#DE0D16")

c25 <- c(
  "dodgerblue2", "#E31A1C", # red
  "green4",
  "#6A3D9A", # purple
  "#FF7F00", # orange
  "black", "gold1",
  "skyblue2", "#FB9A99", # lt pink
  "palegreen2",
  "#CAB2D6", # lt purple
  "#FDBF6F", # lt orange
  "gray70", "khaki2",
  "maroon", "orchid1", "deeppink1", "blue1", "steelblue4",
  "darkturquoise", "green1", "yellow4", "yellow3",
  "darkorange4", "brown"
)

# ## parallelization ##
nCores <- 16
registerDoParallel(nCores)



```



```{r load-theta-data}

## Load Region specific csvs ##
hc_turnaround_df <- read_csv(path(here(), "munge", "theta_hc_all_subs_turnaround.csv"))
amyg_turnaround_df <- read_csv(path(here(), "munge", "theta_amyg_all_subs_turnaround.csv"))
ofc_turnaround_df <- read_csv(path(here(), "munge", "theta_ofc_all_subs_turnaround.csv"))
cing_turnaround_df <- read_csv(path(here(), "munge", "theta_cing_all_subs_turnaround.csv"))
dlpfc_turnaround_df <- read_csv(path(here(), "munge", "theta_dlpfc_all_subs_turnaround.csv"))
insula_turnaround_df <- read_csv(path(here(), "munge", "theta_insula_all_subs_turnaround.csv"))

## Bind together ##
theta_turnaround_df <- rbind(hc_turnaround_df %>% mutate(region = "hc"),
                  amyg_turnaround_df %>% mutate(region = "amyg"),
                  ofc_turnaround_df %>% mutate(region = "ofc"),
                  cing_turnaround_df %>% mutate(region = "cing"),
                  dlpfc_turnaround_df %>% mutate(region = "dlpfc"),
                  insula_turnaround_df %>% mutate(region = "insula"))

theta_turnaround_df <- theta_turnaround_df %>% 
  mutate(trial_time = trial_time -1) %>%
  filter(!grepl("noghost", electrode)) %>%
  mutate(elec_short = gsub("_.*", "", electrode))

```

```{r hc-hc-test}

sub_df <- theta_turnaround_df %>% filter(subject == "SLCH002")
elec1 <- "J4-J5"
elec2 <- "J5-J6"
sub <- "SLCH002" 
true_cor <- NULL
null_cor <- matrix(nrow = length(unique(sub_df$trial_numeric)), ncol = length(unique(sub_df$trial_numeric)))

for(t in 1:length(unique(sub_df$trial_numeric))){
  trial <- unique(sub_df$trial_numeric)[t]
  print(trial)
  t1 <- sub_df  %>% filter(trial_numeric == trial) %>% filter(electrode == "J4-J5_hc_trial_theta_away_locked_ghost.csv") %>% pull(theta)
  t2 <- sub_df  %>% filter(trial_numeric == trial) %>% filter(electrode == "J5-J6_hc_trial_theta_away_locked_ghost.csv") %>% pull(theta)
  true_cor[t] <- cor(t1, t2)
  
  #null distribution
  null_cor[t,] <- foreach(h = 1:length(unique(sub_df$trial_numeric)), .inorder=FALSE, .combine = 'c') %dopar% {
    null_trial <- unique(sub_df$trial_numeric)[h]
    if(null_trial > trial){
      t1 <- sub_df  %>% filter(trial_numeric == trial) %>% filter(electrode == "J4-J5_hc_trial_theta_away_locked_ghost.csv") %>% pull(theta)
      t2 <- sub_df  %>% filter(trial_numeric == null_trial) %>% filter(electrode == "J5-J6_hc_trial_theta_away_locked_ghost.csv") %>% pull(theta)
      return(cor(t1, t2))
    } else {
      return(NA)
    }
  }
}  

# save out files
write.csv(true_cor, path(here(), "results", "cor_analysis", paste(sub, elec1, "to", elec2, "true_cor.csv", sep = "_")))
null_cor <- null_cor[!is.na(null_cor)]
test_result <- wilcox.test(true_cor, null_cor)
write.csv(null_cor, path(here(), "results", "cor_analysis", paste(sub, elec1, "to", elec2, "null_cor.csv", sep = "_")))


```

```{r}

null_cor <- null_cor[!is.na(null_cor)]

test_result <- wilcox.test(true_cor, null_cor)
```


```{r}

true_cor_df <- tibble("true_cor" = true_cor)
null_cor_df <- tibble("null_cor" = null_cor)

ggplot() +
  geom_density(data = null_cor_df, aes(x = null_cor), fill = "grey", color = "black", alpha = .5) +
  geom_density(data = true_cor_df, aes(x = true_cor), fill = "blue", color = "black", alpha = .5) +
  theme(panel.background = element_rect(fill = "white"))

```



# test 2


```{r hc-hc-test}

sub_df <- theta_turnaround_df %>% filter(subject == "SLCH002")
elec1 <- "J6-J7"
elec2 <- "A2-A3"
sub <- "SLCH002" 
true_cor <- NULL
null_cor <- matrix(nrow = length(unique(sub_df$trial_numeric)), ncol = length(unique(sub_df$trial_numeric)))

for(t in 1:length(unique(sub_df$trial_numeric))){
  trial <- unique(sub_df$trial_numeric)[t]
  print(trial)
  t1 <- sub_df  %>% filter(trial_numeric == trial) %>% filter(elec_short == elec1) %>% pull(theta)
  t2 <- sub_df  %>% filter(trial_numeric == trial) %>% filter(elec_short == elec2) %>% pull(theta)
  true_cor[t] <- cor(t1, t2)
  
  #null distribution
  null_cor[t,] <- foreach(h = 1:length(unique(sub_df$trial_numeric)), .inorder=FALSE, .combine = 'c') %dopar% {
    null_trial <- unique(sub_df$trial_numeric)[h]
    if(null_trial > trial){
      t1 <- sub_df  %>% filter(trial_numeric == trial) %>% filter(elec_short == elec1) %>% pull(theta)
      t2 <- sub_df  %>% filter(trial_numeric == null_trial) %>% filter(elec_short == elec2) %>% pull(theta)
      return(cor(t1, t2))
    } else {
      return(NA)
    }
  }
}  

# save out files
write.csv(true_cor, path(here(), "results", "cor_analysis", paste(sub, elec1, "to", elec2, "true_cor.csv", sep = "_")))
null_cor <- null_cor[!is.na(null_cor)]
test_result <- wilcox.test(true_cor, null_cor)
write.csv(null_cor, path(here(), "results", "cor_analysis", paste(sub, elec1, "to", elec2, "null_cor.csv", sep = "_")))

test_result <- wilcox.test(true_cor, null_cor)
test_result
```

```{r}

true_cor_df <- tibble("true_cor" = true_cor)
null_cor_df <- tibble("null_cor" = null_cor)

ggplot() +
  geom_density(data = null_cor_df, aes(x = null_cor), fill = "grey", color = "black", alpha = .5) +
  geom_density(data = true_cor_df, aes(x = true_cor), fill = "blue", color = "black", alpha = .5) +
  theme(panel.background = element_rect(fill = "white"))

```


```{r hc-cing}

sub_df <- theta_turnaround_df %>% filter(subject == "SLCH002")
elec1 <- "J6-J7"
sub <- "SLCH002"

cing_elecs <- sub_df %>%
  filter(region == "cing") %>%
  pull(elec_short) %>%
  unique()

elec1_df <- sub_df %>% filter(elec_short == elec1)

tests <- tibble(subject = character(), elec1 = character(), elec2 = character(), pval = numeric())
for(elec2 in cing_elecs){
  
  # filter to elec 2
  elec2_df <- sub_df %>% filter(elec_short == elec2)
    
  # prep output
  true_cor <- NULL
  null_cor <- matrix(nrow = length(unique(sub_df$trial_numeric)), ncol = length(unique(sub_df$trial_numeric)))
  for(t in 1:length(unique(sub_df$trial_numeric))){
    trial <- unique(sub_df$trial_numeric)[t]
    print(trial)
    t1 <- elec1_df  %>% filter(trial_numeric == trial)  %>% pull(theta)
    t2 <- elec2_df  %>% filter(trial_numeric == trial) %>% filter(elec_short == elec2) %>% pull(theta)
    true_cor[t] <- cor(t1, t2)
    
    #null distribution
    null_cor[t,] <- foreach(h = 1:length(unique(sub_df$trial_numeric)), .inorder=FALSE, .combine = 'c') %dopar% {
      null_trial <- unique(sub_df$trial_numeric)[h]
      if(null_trial > trial){
        t1 <- elec1_df  %>% filter(trial_numeric == trial)  %>% pull(theta)
        t2 <- elec2_df  %>% filter(trial_numeric == null_trial) %>%  pull(theta)
        return(cor(t1, t2))
      } else {
        return(NA)
      }
    }
  }  
  
  # compute difference
  null_cor <- null_cor[!is.na(null_cor)]
  test_result <- wilcox.test(true_cor, null_cor)
  tests <- rbind(tests, c(sub, elec1, elec2, test_result$p.value))
  
  # save out files
  write.csv(true_cor, path(here(), "results", "cor_analysis", paste(sub, elec1, "to", elec2, "true_cor.csv", sep = "_")))
  write.csv(null_cor, path(here(), "results", "cor_analysis", paste(sub, elec1, "to", elec2, "null_cor.csv", sep = "_")))
  write.csv(tests, path(here(), "results", "cor_analysis", paste0(sub, "_correlation_wilcox_test.csv")))
  

}
```


```{r cing-mean-correl}

sub_list <- c("LL10", "LL12", "LL13", "BJH021", "BJH025", "SLCH002", "BJH016")
cing_correlation <- tibble(subject = character(), elec1 = character(), elec2 = character(), mean_correl = numeric())
for(sub in sub_list){

  sub_df <- theta_turnaround_df %>% filter(subject == sub)
  
  hc_elecs <- sub_df %>%
    filter(region == "hc") %>%
    pull(elec_short) %>%
    unique()
  
  cing_elecs <- sub_df %>%
    filter(region == "cing") %>%
    pull(elec_short) %>%
    unique()
    
  for(elec1 in hc_elecs){
    for(elec2 in cing_elecs){
      
      if(sub == "SLCH002" & elec2 == "B4-B5"){
        next 
      }
      
      true_cor_df <- read_csv(path(here(), "data_mount/remote/pacman_behavior", 
                                   "results", "cor_analysis", paste(sub, elec1, "to", elec2, "true_cor.csv", sep = "_")))
      # null_cor_df <- read_csv(path(here(), "data_mount/remote/pacman_behavior", 
      #                              "results", "cor_analysis", paste(sub, elec1, "to", elec2, "null_cor.csv", sep = "_")))
      
      cing_correlation <- rbind(cing_correlation, c(sub, elec1, elec2, mean(true_cor_df$x)))
    
    }
  
  }

}

```



```{r dlpfc-mean-correl}

sub_list <- c("LL10", "LL12", "LL13", "BJH021", "BJH025", "SLCH002", "BJH016")
dlpfc_correlation <- tibble(subject = character(), elec1 = character(), elec2 = character(), mean_correl = numeric())
for(sub in sub_list){

  sub_df <- theta_turnaround_df %>% filter(subject == sub)
  
  hc_elecs <- sub_df %>%
    filter(region == "hc") %>%
    pull(elec_short) %>%
    unique()
  
  dlpfc_elecs <- sub_df %>%
    filter(region == "dlpfc") %>%
    pull(elec_short) %>%
    unique()
    
  for(elec1 in hc_elecs){
    for(elec2 in dlpfc_elecs){
      
      true_cor_df <- read_csv(path(here(), "data_mount/remote/pacman_behavior", 
                                   "results", "cor_analysis", paste(sub, elec1, "to", elec2, "true_cor.csv", sep = "_")))
      # null_cor_df <- read_csv(path(here(), "data_mount/remote/pacman_behavior", 
      #                              "results", "cor_analysis", paste(sub, elec1, "to", elec2, "null_cor.csv", sep = "_")))
      
      dlpfc_correlation <- rbind(dlpfc_correlation, c(sub, elec1, elec2, mean(true_cor_df$x)))
    
    }
  
  }

}
```

```{r}
colnames(dlpfc_correlation) <- c("subject", "elec1", "elec2", "correlation")
colnames(cing_correlation) <- c("subject", "elec1", "elec2", "correlation")

dlpfc_correlation <- dlpfc_correlation %>%
  mutate(correlation = as.numeric(correlation)) 
cing_correlation <- cing_correlation %>%
  mutate(correlation = as.numeric(correlation))

wilcox.test(dlpfc_correlation$correlation, cing_correlation$correlation)

ggplot() +
  geom_density(data = dlpfc_correlation, aes(x = correlation), fill = "#FFD680", color = "black", alpha = .5) +
  geom_density(data = cing_correlation, aes(x = correlation), fill = "#CC0029", color = "black", alpha = .5) +
  theme(panel.background = element_rect(fill = "white")) +
  ggtitle("Average dlPFC vs Ant. Cingulate Connectivity w/Hipp.")


```
```{r}

correlation_df <- rbind(dlpfc_correlation %>% mutate(region = "dlpfc"),
                        cing_correlation %>% mutate(region = "cing"))


test_correlation_df <- correlation_df %>%
  group_by(subject) %>%
  mutate(pval = wilcox.test(correlation ~ region)$p.value) %>%
  select(subject, region, pval) %>%
  distinct() %>%
  mutate(sig = pval < .05)

wilcox.test(dlpfc_correlation$correlation, cing_correlation$correlation)


model <- lmerTest::lmer(correlation ~ region + (1|subject), data = correlation_df)
summary(model)

```




```{r visualize}
sub <- "BJH016"
elec1 <- "HL5-HL6"
elec2 <- "CL9-CL10"
true_cor_df <- read_csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", paste(sub, elec1, "to", elec2, "true_cor.csv", sep = "_")))
null_cor_df <- read_csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", paste(sub, elec1, "to", elec2, "null_cor.csv", sep = "_")))

ggplot() +
  geom_density(data = null_cor_df, aes(x = x), fill = "grey", color = "black", alpha = .5) +
  geom_density(data = true_cor_df, aes(x = x), fill = "blue", color = "black", alpha = .5) +
  theme(panel.background = element_rect(fill = "white"))

```


```{r count-dlpfc}

dlpfc_counter <- c()

# LL10 dlPFC #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "LL10_correlation_wilcox_test_dlpfc.csv"), header = T)
tmp <- results %>%
  filter(X != 1) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
sum(as.numeric(tmp$pval_fdr < .05))/nrow(tmp)
dlpfc_counter <- c(dlpfc_counter, tmp$pval_fdr < .05)

# LL12 dlPFC #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "LL12_correlation_wilcox_test_hc_dlpfc.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
sum(as.numeric(tmp$pval_fdr < .05))/nrow(tmp)
dlpfc_counter <- c(dlpfc_counter, tmp$pval_fdr < .05)

# LL13 dlPFC #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "LL13_correlation_wilcox_test_hc_dlpfc.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
sum(as.numeric(tmp$pval_fdr < .05))/nrow(tmp)
dlpfc_counter <- c(dlpfc_counter, tmp$pval_fdr < .05)

# BJH021 dlPFC #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "BJH021_correlation_wilcox_test_hc_dlpfc.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
sum(as.numeric(tmp$pval_fdr < .05))/nrow(tmp)

dlpfc_counter <- c(dlpfc_counter, tmp$pval_fdr < .05)


# BJH025 dlPFC #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "BJH025_correlation_wilcox_test_hc_dlpfc.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
sum(as.numeric(tmp$pval_fdr < .05))/nrow(tmp)
dlpfc_counter <- c(dlpfc_counter, tmp$pval_fdr < .05)

# SLCH002 dlPFC #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "SLCH002_correlation_wilcox_test_hc_dlpfc.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
sum(as.numeric(tmp$pval_fdr < .05))/nrow(tmp)
dlpfc_counter <- c(dlpfc_counter, tmp$pval_fdr < .05)


# BJH016 dlPFC #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "BJH016_correlation_wilcox_test_hc_dlpfc.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1) & V1 != 1) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
sum(as.numeric(tmp$pval_fdr < .05))/nrow(tmp)
dlpfc_counter <- c(dlpfc_counter, tmp$pval_fdr < .05)

```

```{r count-cing}



# LL10 cing #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "LL10_correlation_wilcox_test_cing.csv"), header = T)
tmp <- results  %>%
  filter(X != 1) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))

table(tmp$pval_fdr < .05)

cing_counter <- c(tmp$pval_fdr < .05)

# LL12 cing #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "LL12_correlation_wilcox_test_hc_cing.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)

cing_counter <- c(cing_counter, tmp$pval_fdr < .05)

# LL13 cing #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "LL13_correlation_wilcox_test_hc_cing.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
cing_counter <- c(cing_counter, tmp$pval_fdr < .05)

# BJH021 cing #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "BJH021_correlation_wilcox_test_hc_cing.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
cing_counter <- c(cing_counter, tmp$pval_fdr < .05)

# BJH025 cing #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "BJH025_correlation_wilcox_test_hc_cing.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
cing_counter <- c(cing_counter, tmp$pval_fdr < .05)

# SLCH002 cing #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "SLCH002_correlation_wilcox_test_hc_cing.csv"), header = T)
tmp <- results %>%
  filter(X != 1) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
cing_counter <- c(cing_counter, tmp$pval_fdr < .05)


# BJH016 cing #
results <- read.csv(path(here(), "data_mount/remote/pacman_behavior", "results", "cor_analysis", "BJH016_correlation_wilcox_test_hc_cing.csv"), header = F)
tmp <- results %>%
  filter(!is.na(V1)) %>%
  mutate(V5 = as.numeric( V5)) %>%
  mutate(pval_fdr = p.adjust(V5, "fdr"))
table(tmp$pval_fdr < .05)
cing_counter <- c(cing_counter, tmp$pval_fdr < .05)


```




