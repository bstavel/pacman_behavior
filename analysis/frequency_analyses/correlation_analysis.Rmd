...
theta_onset_df <- theta_onset_df %>% 
  ...
  mutate(elec_short = gsub("_.*", "", "J4-J5_hc_trial_theta_away_locked_ghost.csv"))

```
null_cor <- foreach(h in unique(sub_df$trial_numeric), .inorder=FALSE, .combine = 'rbind') %dopar% {
