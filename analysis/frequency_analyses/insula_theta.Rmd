...
avg_insula_theta_data <- insula_theta_data %>%
  rename(theta = theta) %>%
...
avg_insula_theta_behave_df <- insula_theta_behave_df %>%
  group_by(trial_time, electrode, subject) %>%
...
insula_theta_lme_df <- insula_theta_behave_df %>%
...
for(step in sort(unique(insula_theta_lme_df$move_time))){
  
  print(step)

  time_step_df <- insula_theta_lme_df %>%
...
# merge behavior data with ieeg data
insula_theta_behave_df <- left_join(all_subs_g_dist %>% select(-move_step) %>% mutate(trial_time = round(trial_time, 2)), 
                               insula_theta_data %>% mutate(trial_time = round(trial_time, 2)))
...
model_results_whole_trial <- rbind(dist_to_g_results_only %>% mutate(case = "dist_only") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))),
                       dist_to_g_results %>% mutate(case = "dist") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))),
                       points_r_results_only %>% mutate(case = "points_only") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))),
                       points_r_results %>% mutate(case = "points") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))))
...
plot <- model_results_whole_trial %>%
  ...
  ggtitle("Insula Theta, Trial Onset")

ggsave(path(here(), "figures", "linear_mixed_effect_models", "insula_theta_trial_onset.png"),
       plot = plot,
       width = 12,
       height = 8,
       units = "in",
...
model_results_onset_before_turn <- rbind(dist_to_g_results_only %>% mutate(case = "dist_only") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))),
                       dist_to_g_results %>% mutate(case = "dist") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))),
                       points_r_results_only %>% mutate(case = "points_only") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))),
                       points_r_results %>% mutate(case = "points") %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time))))
...
plot <- model_results_onset_before_turn %>%
  ...
  scale_color_manual(values = c("#87C8B7", "white", "#FCC673", "black")) +
  xlim(0, 3.5) +
  ggtitle("insuladala Theta, Trial Onset, Before Turn")


ggsave(path(here(), "figures", "linear_mixed_effect_models", "insula_theta_trial_onset_before_turn.png"),
       plot = plot,
       width = 12,
       height = 8,
       units = "in",
...
for(step in sort(unique(insula_theta_lme_df$turn_time))){
...
model_results <- rbind(dist_to_g_results_only %>% mutate(case = "dist_only") %>% mutate(turn_time = sort(unique(insula_theta_lme_df$turn_time))),
                       dist_to_g_results %>% mutate(case = "dist") %>% mutate(turn_time = sort(unique(insula_theta_lme_df$turn_time))),
                       points_r_results_only %>% mutate(case = "points_only") %>% mutate(turn_time = sort(unique(insula_theta_lme_df$turn_time))),
                       points_r_results %>% mutate(case = "points") %>% mutate(turn_time = sort(unique(insula_theta_lme_df$turn_time))))
...
plot <- model_results %>%
  ...
  scale_color_manual(values = c("#87C8B7", "white", "#FCC673", "black"))  +
  xlim(-2, 2) +
  ggtitle("Insula Theta, Turnaround")

ggsave(path(here(), "figures", "linear_mixed_effect_models", "insula_theta_trial_turnaround.png"),
       plot = plot,
       width = 12,
       height = 8,
       units = "in",
...
model_results_userloc <- userloc_results_only %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time)))
...
plot <- model_results_userloc %>%
  ...
  ggtitle("insuladala Theta, Pacman Location")


ggsave(path(here(), "figures", "linear_mixed_effect_models", "insula_theta_trial_pacman_location.png"),
       plot = plot,
       width = 12,
       height = 8,
       units = "in",
...
model_results_ghostloc <- ghostloc_results_only %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time)))
                       

plot <- model_results_ghostloc %>%
  mutate(pval_fdr = p.adjust(`Pr(>|t|)`, "fdr")) %>%
  mutate(sig = if_else(pval_fdr < .05, "Sig", "Not Sig.")) %>%
  ggplot(., aes(x = move_time, y = Estimate)) +
  ...
  scale_color_manual(values = c("white", "black")) +
  xlim(0, 3.5) +ylim(-1, 1) +
  ggtitle("insuladala Theta, Ghost Location")


ggsave(path(here(), "figures", "linear_mixed_effect_models", "insula_theta_trial_ghost_location.png"),
       plot = plot,
       width = 12,
       height = 8,
       units = "in",
...
model_results_value <- value_results_only %>% mutate(move_time = sort(unique(insula_theta_lme_df$move_time)))
                       

plot <- model_results_value %>%
  mutate(sig = if_else(`Pr(>|t|)` < .05, "Sig", "Not Sig.")) %>%
  ggplot(., aes(x = move_time, y = Estimate)) +
  geom_ribbon(aes(ymin = Estimate - 1.96*`Std. Error`, ymax = Estimate + 1.96*`Std. Error`), alpha = .3, fill = "#D1B5F4") +
  geom_point(aes(color = sig), size = 6, shape = 21, fill = "#D1B5F4") +
  geom_line(color = "#D1B5F4") +
  theme(panel.background = element_rect(fill = "white"), legend.position = "top") +
  scale_color_manual(values = c("white", "black")) +
  xlim(0, 3.5) +ylim(-2, 2) +
  ggtitle("Insula Theta, Combined Value")


ggsave(path(here(), "figures", "linear_mixed_effect_models", "insula_theta_trial_combined_value.png"),
       plot = plot,
       width = 12,
       height = 8,
       units = "in",
...
model_1 <- lmer(theta ~ combined_value + (1|subject/elec_id), data = time_step_df)

model_2 <- lm(theta ~ combined_value, data = time_step_df)

summary(model_1)  
summary(model_2)  

random_effs <- ranef(model_1)
fixed_effs <- fixef(model_1)


plot_model(model_1, type = "est")
plot_model(model_1, type = "re")


(pp <- plot_model(model_1,type="pred",
       terms=c("combined_value","subject"),pred.type="re")) +
  ylim(0, 2)
...
# by subject
summary(lm(theta ~ combined_value, data = time_step_df %>% filter(subject == "BJH016")))
summary(lm(theta ~ combined_value, data = time_step_df %>% filter(subject == "BJH021")))
summary(lm(theta ~ combined_value, data = time_step_df %>% filter(subject == "BJH025")))
summary(lm(theta ~ combined_value, data = time_step_df %>% filter(subject == "SLCH002")))
summary(lm(theta ~ combined_value, data = time_step_df %>% filter(subject == "LL12")))
...
write_csv(model_results_whole_trial, path(here(), "results", "insula_theta_model_results_whole_trial.csv"))
write_csv(model_results, path(here(), "results", "insula_theta_model_results_turnarouns.csv"))
write_csv(model_results_value, path(here(), "results", "insula_theta_model_results_integrated_value.csv"))
