...
# rename var #
ofc_hfa_data <- ofc_hfa_data %>% rename(hfa = theta)
...
avg_ofc_hfa_behave_df <- ofc_hfa_behave_df %>%
  ...
  mutate(mean_elec_hfa = mean(hfa, na.rm = T)) %>%
  mutate(mean_hfa_upper = t.test(hfa)$conf.int[2]) %>%
  mutate(mean_hfa_lower = t.test(hfa)$conf.int[1])
  ungroup() #%>%
  # select(-trial_numeric, -hfa) %>%
  # distinct() %>%
  # group_by(reward_groups, trial_time, subject) %>%
  # mutate(mean_sub_hfa = mean(mean_elec_hfa, na.rm = T)) %>%
  # mutate(mean_hfa_upper = t.test(mean_elec_hfa)$conf.int[2]) %>%
  # mutate(mean_hfa_lower = t.test(mean_elec_hfa)$conf.int[1])
  # ungroup() # %>%
  # select(-mean_elec_hfa, -electrode) %>%
  # distinct() %>%
...
avg_ofc_hfa_behave_df %>%
  mutate(elec_id = paste0(subject, "_", gsub("_.*", "", electrode))) %>%
  ggplot(., aes(x = trial_time, y = mean_elec_hfa, color = factor(reward_groups), fill = factor(reward_groups))) +
  geom_point() +
  geom_line() +
  geom_ribbon(aes(ymin = mean_hfa_lower, ymax = mean_hfa_upper), alpha = .5) +
  theme(panel.background = element_rect(fill = "white")) +
