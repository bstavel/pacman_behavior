...
conn_diff_plots <- function(results, region_list, title){
  ...
  p <- ggplot(results_plot_df) +
    geom_vline(xintercept = 0, color = "black") +
    geom_density(aes(x = sample_diff, fill = direction, color = direction), alpha = .25) +
    geom_segment(aes(x = Mean, xend = Mean, y = 0, yend = max_density, color = direction), linewidth = .5, linetype = "dotdash",) +
    geom_segment(aes(x = ci_upper, xend = ci_upper, y = 0, yend = upper_density, color = direction),  linewidth = .5) +
    geom_segment(aes(x = ci_lower, xend = ci_lower, y = 0, yend = lower_density, color = direction),  linewidth = .5) +
    geom_text(aes(label = sig, color = direction, min(results_plot_df$sample_diff), y = max(results_plot_df$max_density)), ,
              hjust = 0, vjust =1, size =10, check_overlap = T) +
    facet_grid(Region2 ~ Region1) +
    ...
    labs( y = "density", x = "difference in mean coherence") +
...
```{r plot, fig.width = 18, fig.height = 10, warning = F}

amyg_plot <- conn_diff_plots(amyg_results, c( "Hipp.", "OFC", "ACC", "MFG"), "Amygdala")
ofc_plot <- conn_diff_plots(ofc_results, c("Amyg.", "Hipp.", "ACC", "MFG"), "OFC")
acc_plot <- conn_diff_plots(acc_results, c("Amyg.", "Hipp.", "OFC",  "MFG"), "ACC")
mfg_plot <- conn_diff_plots(mfg_results, c("Amyg.", "Hipp.", "OFC", "ACC"), "MFG")


grid.arrange(amyg_plot, hc_plot, ofc_plot, acc_plot, mfg_plot, ncol = 3)
...
hc_plot <- conn_diff_plots(hc_results, c("Amyg.", "OFC", "ACC", "MFG"), "Hippocampus")
hc_plot

ggsave(path(here(), "figures", "connectivity", "figure3_hippocampus_coherence_example.png"), 
       plot = hc_plot, 
...
ggsave(path(here(), "figures", "connectivity", "figure3_differential_theta_coherence_tiles.png"), 
...
example_elec_data <- amyg_model_data %>%
...
example_sub_data <- example_elec_data %>%
  group_by(subject, second_region) %>%
  summarise( mean_sub_coherenece = mean(mean_elec_coherence, na.rm = TRUE)) 


example_plot <- example_elec_data %>%
  mutate(second_region = fct_relevel(second_region, "Hippocampus", "MFG")) %>%
  ggplot(., aes(x = second_region, y = mean_elec_coherence)) +
  geom_violin(aes(fill = second_region), color = "#2D2327") +
  geom_boxplot(aes(fill = second_region), color = "#2D2327", width = .3, notch = T) +
  geom_jitter(data = example_sub_data, aes(y = mean_sub_coherenece, fill = second_region), color = "#2D2327", shape = 25, width = .1, size = .75) +
  theme_minimal() +
  theme(axis.text = element_text(family = "Gill Sans", color = "#2D2327", size = 7),
        axis.title = element_text(family = "Gill Sans", color = "#2D2327", size = 7.5),
        legend.title = element_text(family = "Gill Sans", color = "#2D2327", size = 7),
        legend.text  = element_text(family = "Gill Sans", color = "#2D2327", size = 7),
        plot.title = element_text(family = "Gill Sans", color = "#2D2327", face = 'bold', size = 9, hjust = 0.5, margin = margin(b= 5)),
  ) +
  scale_fill_manual(values = c( "#DBE317", "#28AE7E"), guide = F) +
  labs(
    x = "",
...
ggsave(path(here(), "figures", "connectivity", paste0("figure3_differential_theta_coherence_example.png")), 
       plot = example_plot, 
