...
imcoh_app_sig_df <- app_sig_elec_df %>%
  ...
  ungroup() %>%
  select(subject, time, scaled_log_connectivity, scale_time, connectivity, pairs, first_pair, second_pair, roi_pair, first_region, second_region, key) %>%
  distinct()
...
priors <- c(
  ...
  prior(gamma(2, 0.1), class = "nu")          # Prior for skewness parameter
)


# Fit the model
rising_model <- brm(
  formula = scaled_log_connectivity ~ scale_time + (1 + time | key),
...
# save rising model #
save(rising_model, file = path(here(), "results", paste0(avd_time_window, "_student_rising_model_brms.RData")))

# load rising model #
load(path(here(), "results", paste0(avd_time_window, "_student_rising_model_brms.RData")))
...
ppc_dens_overlay(rising_model$data$scaled_log_connectivity, y_rep) + xlim(-10, 10)
...
ppc_hist(imcoh_app_sig_df$scaled_log_connectivity, y_rep[1:5, ])
...
ppc_stat(imcoh_app_sig_df$scaled_log_connectivity, y_rep, stat = "max")
...
# Fit the model
falling_model <- brm(
  formula = scaled_log_connectivity ~ time + (1 + scale_time | subject/key),
...
# save falling model #
save(falling_model, file = path(here(), "results", paste0(avd_time_window, "_student_falling_model_brms.RData")))

# load falling model #
load(path(here(), "results", paste0(avd_time_window, "_student_falling_model_brms.RData")))
...
# Generate replicated datasets
y_rep <- posterior_predict(falling_model, ndraws = 100)

ppc_dens_overlay(imcoh_avd_sig_df$scaled_log_connectivity, y_rep) 
...
ppc_dens_overlay_grouped(imcoh_avd_sig_df$scaled_log_connectivity, y_rep, imcoh_avd_sig_df$subject)
...
ppc_scatter_avg(imcoh_avd_sig_df$scaled_log_connectivity, y_rep[1:50, ])
...
ppc_stat(imcoh_avd_sig_df$scaled_log_connectivity, y_rep, stat = "mean")
ppc_stat(imcoh_avd_sig_df$scaled_log_connectivity, y_rep, stat = "sd")
ppc_stat(imcoh_avd_sig_df$scaled_log_connectivity, y_rep, stat = "min")
ppc_stat(imcoh_avd_sig_df$scaled_log_connectivity, y_rep, stat = "max")
