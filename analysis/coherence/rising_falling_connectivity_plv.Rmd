---
title: "Rising/Falling Connectivity - pli"
...
```{r load-data}
## load data ##
conn_df <- read_csv(path(here(), "data_mount", "remote", "pacman", "connectivity", "ieeg", "pli_pli_pli", "combined_ghost_connectivity.csv"))
...
```{r pli-app-eda, fig.width = 10, fig.height = 5}

## do some basic approach df EDA ##

pli_app_sig_df <- app_sig_elec_df %>%
  ungroup() %>%
...
summary(lmer(scaled_log_connectivity ~ time+ (1 + time | subject/key), data = pli_app_sig_df))
...
# save rising model #
save(rising_model, file = path(here(), "results", "rising_model_brms_pli.RData"))

# load rising model #
load(path(here(), "results", "rising_model_brms_pli.RData"))
...
pli_dens_overlay(y = pli_app_sig_df$scaled_log_connectivity,
...
pli_avd_sig_df <- avd_sig_elec_df %>%
  ungroup() %>%
  filter(metric == "Phase Lag Index") %>%
  filter(first_region != "SFG" & second_region != "SFG") %>%
  filter(roi_pair != "mfg_mfg") %>%
...
summary(lmer(scaled_log_connectivity ~ time+ (1 + time | subject/key), data = pli_avd_sig_df))
...
# save falling model #
save(falling_model, file = path(here(), "results", "falling_model_brms_pli.RData"))

# load rising model #
load(path(here(), "results", "falling_model_brms_pli.RData"))
...
pli_app_sig_avg_df <- app_sig_avg_elec_df %>%
  filter(metric == "Phase Lag Index") %>%
  filter(first_region != "sfg" & second_region != "sfg") %>%
  filter(roi_pair != "mfg_mfg") %>%
...
pli_avd_sig_avg_df <- avd_sig_avg_elec_df %>%
  filter(metric == "Phase Lag Index") %>%
  filter(first_region != "sfg" & second_region != "sfg") %>%
  filter(roi_pair != "mfg_mfg") %>%
  select(subject, time, average_sub_conn, roi_pair, first_region, second_region) %>%
...
pli_sig_avg_df <- bind_rows(pli_avd_sig_avg_df %>% mutate(case = "Avoid"), 
                              pli_app_sig_avg_df %>% mutate(case = "Approach"))

# take the absolute value of time #
indv_pli_sig_avg_plot_df <- pli_sig_avg_df %>%
  mutate(plot_time = abs(time))

# plot #
indv_pli_sig_avg_plot <- indv_pli_sig_avg_plot_df %>%
  ...
  ggtitle("Theta Coherence Rises During Approach and Falls During Avoidance", subtitle = "Signficiance assessed using bayesian linear mixed effects models of Phase Lag Index")

# save #
ggsave(path(here(), "figures", "connectivity", "rising_falling_indv_plot_pli.png"), 
       plot = indv_pli_sig_avg_plot, 
...
# take the absolute value of time #
pli_sig_avg_plot_df <- pli_sig_avg_df %>%
  mutate(plot_time = abs(time)) %>%
...
# plot #
pli_sig_avg_plot <- pli_sig_avg_df %>%
  ggplot(., aes(x = time, y = log(average_sub_conn), color = case, fill = case)) +
  geom_jitter(alpha = .5) +
  geom_smooth(method = "lm", color = "black") +
...
# save #
ggsave(path(here(), "figures", "connectivity", "rising_falling_plot_pli.png"), 
       plot = pli_sig_avg_plot, 
