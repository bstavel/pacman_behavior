...
imcoh_clean_sig_roi_average_df <- imcoh_clean_sig_average_df %>%
  filter(first_region != "SFG") %>%
  select(-average_sub_conn, -subject) %>%
  distinct() %>%
  group_by(first_region, time) %>%
  mutate(first_region_average = mean(average_conn)) %>%
  ungroup() %>%
  select(-average_conn, -second_region, -detailed_second_region, -roi_pair) %>%
  distinct()

imcoh_clean_sig_roi_average_df %>%
  ggplot(., aes(x=time, y = first_region_average, color = first_region)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "#2D2327") +
  geom_line(size = 1) +
  scale_alpha_manual(values = c(1, .3)) +
  scale_color_manual(values = roi_colors) + 
