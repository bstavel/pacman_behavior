...
sig_10_list <- sub_elec_lists %>% filter(threshold == "10") %>% distinct()
...
avd_sig_elec_df <- conn_detailed_df %>%
  ...
  filter(key %in% sig_10_list$pair_id) %>%
...
# save falling model #
save(roi_falling_model, file = path(here(), "results", "2_roi_falling_model_brms.RData"))
