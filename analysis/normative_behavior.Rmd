...
```{r dsitance-with-or-without-chaseattack-trials}
...
clean_distance_bob_data <- pilot_game_data_distance %>%
  ...
  mutate(dist_ghost_bob = distance_to_ghost) %>%
  select(subject, trial_numeric, dist_ghost_bob)
...
distance_compare %>%
  ...
  pivot_longer(cols = c(distance_to_ghost, dist_ghost_bob), values_to = "distance", names_to = "type") %>%
  ggplot(., aes(x = subject, y = distance, fill = type )) +
  geom_boxplot(notch = T) +
  theme(panel.background = element_rect(fill = "white"))
