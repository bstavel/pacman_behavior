...
dots_plot <- all_vars_df %>%
  ...
  geom_point(data = ieeg_dot_df, aes(x = factor(block), y = average_dots_per_block), color = "#FB6087", fill = 'darkgrey', size = 3, shape = 23) +
 theme(panel.background = element_rect(fill = "white"), 
  ...
  labs(subtitle = "Participants balanced the rewards against the risks by collecting most, but not all,\nof the reward on a given trial", x = "\nBlock (each block consists of twenty trials)", y = "Average dots collected\n") +
  ggtitle("Average reward collected across blocks")

dots_plot

ggsave(path(here(), "figures", "behavior", "figure1_dots_plot.png"), 
       plot = dots_plot, width = 9.5, height = 7)
...
last_away_plot <- last_away_df %>%
  ...
  ylab("Distance to ghost at final turnaround (a.u.)") + xlab("\n") +
  theme(panel.background = element_rect(fill = "white"),
        legend.position = "top",
        plot.subtitle = element_text(family = "Gill Sans", color = '#2D2327', size = 18),
        axis.title = element_text(family = "Gill Sans", color = '#2D2327', size = 20),
  ...
  ggtitle("Variability in risk tolderance", subtitle = "Random sample of 20 participants shows across- and within- subject variability\nin the degree of risk incurred on a given trial")

last_away_plot

ggsave(path(here(), "figures", "behavior", "figure1_last_away_plot.png"), 
       plot = last_away_plot, width = 9.5, height = 7)
...
turn_reward_plot <- turn_reward_df %>%
  ...
  geom_boxplot(aes(, fill = large_reward), notch = T, show.legend = F) +
  geom_jitter(aes(color = ieeg)) +
  ylim(-1.25, 1.25) +
  coord_flip() +
  theme(panel.background = element_rect(fill = "white"), 
  ...
  scale_color_manual(values = c("#2D2327", "darkgrey")) +
...
experienced_plot <- phenom_df %>%
  ...
  xlab("") +
  ylab("Percent of Prolific Participants (n = 191)") +
  theme(panel.background = element_rect(fill = "white"), 
        legend.position = "none",
