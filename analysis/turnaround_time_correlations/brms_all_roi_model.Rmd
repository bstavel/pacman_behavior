...
# correlation_sig_df <- correlation_sig_df %>%
#   filter(!is.na(dots_eaten)) %>%
#   mutate(scaled_dots = scale(dots_eaten)[,1])

correlation_sig_df <- correlation_sig_df %>%
  filter(!is.na(last_away)) %>%
  mutate(last_away = scale(last_away)[,1])

# Fit the model
model <- brm(
  formula = scale_correlation ~ last_away * region_pair + (1 + last_away | subject/roi_pair1),
...
# save full model #
save(model, file = path(here(), "results", "full_all_roi_distance_model_brms_newsubs_noinsula.RData"))
...
region_pairs <- unique(correlation_sig_df$region_pair)
  ...
  get_samples_for_region <- function(region) {
    if (region == "amyg_cing") {
      slope <- as_draws_df(model)$b_last_away
    } else {
      interaction_term <- paste0("b_last_away:region_pair", region)
      slope <- as_draws_df(model)$b_last_away + as_draws_df(model)[[interaction_term]]
    }
  }
