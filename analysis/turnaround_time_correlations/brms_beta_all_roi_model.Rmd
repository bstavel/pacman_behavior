---
title: "Beta Correlations & Time to Turnaround"
...
# load correlations #
correlation_df <- read_csv(path(here(), "results", "beta_correlations_newsubs.csv"))
...
```
Maybe increase beta iter on the next run!
...
# save full model #
save(model, file = path(here(), "results", "full_beta_all_roi_model_brms_newsubs.RData"))

# load full model #
load(path(here(), "results", "full_beta_all_roi_model_brms_newsubs.RData"))
...
full_sample_plot_df <- full_sample_df %>%
  ...
  mutate(Region_Pair = factor(Region_Pair, 
                              levels = c("OFC ~ Amyg.", "OFC ~ HC", "OFC ~ ACC", "OFC ~ Insula", "OFC ~ MFG", 
                                         "HC ~ Amyg.", "HC ~ ACC", "HC ~ Insula", "HC ~ MFG", "Insula ~ ACC", 
                                         "Amyg. ~ ACC", "Amyg. ~ Insula", "Amyg. ~ MFG", "MFG ~ ACC", "MFG ~ Insula")))
...
turn_time_plot <- full_sample_plot_df %>%
  ...
  ylim(0, 60) +
  ggtitle("Turnaround Time Correlates with Trial-by-Trial beta Connectivity", subtitle = "Each facet shows the posterior distribution for the slope for each region pair, * indicates the 95% CI did not include 0")

ggsave(path(here(), "figures", "correl_turn_predictions", "beta_turn_time_plot_newsubs.png"),
       plot = turn_time_plot,
