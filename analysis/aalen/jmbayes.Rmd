...
permuted_sub_df <- pilot_game_data_distance %>%
  ...
  mutate(points_remaining = sample(permuted_sub_df$points_remaining, length(points_remaining))) %>%
...
permuted_sub_cox_df <- permuted_sub_df %>%
  ...
  ungroup() %>%
  mutate(event = 1) %>%
  select(trial_numeric, Time, event) %>%
...
# longitudinal model #
MixedModelFit2 <- mvglmer(list(distance_to_ghost ~ trial_time + (trial_time | trial_numeric),
                               points_remaining ~ trial_time + (trial_time | trial_numeric)), 
                          data = permuted_sub_df, families = list(gaussian, gaussian))

# survival model #
CoxFit <- coxph(Surv(Time, event) ~ 1, data = permuted_sub_cox_df, model = TRUE)


JMFit1 <- mvJointModelBayes(MixedModelFit2, CoxFit, timeVar = "trial_time")
summary(JMFit1)
...
roc <- rocJM(JMFit1, test_permuted_sub_df, Tstart = .5, Dt = 1, idVar = 'trial_numeric')
plot(roc)   

aucJM(JMFit1, permuted_sub_df, Tstart = .5, Dt = 1, idVar = 'trial_numeric')
...
lastR_lme <- lme(dist_last_big_r ~ trial_time, data = trian_sub_long_df, random = ~trial_time | trial_numeric)

# survival model #
CoxFit <- coxph(Surv(Time, EVENT) ~ 1, data = sub_cox_df)

# joint model #
JMFit1 <- jm(CoxFit, list(distGhost_lme, lastR_lme), time_var = "trial_time", data_Surv = sub_cox_df, n_burnin = 1000, n_iter = 30000, n_chains =8)
summary(JMFit1)
